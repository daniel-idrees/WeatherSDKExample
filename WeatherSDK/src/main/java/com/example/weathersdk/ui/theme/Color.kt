package com.example.weathersdk.ui.theme

import androidx.compose.ui.graphics.Color

/**
 * Internal use only.
 */

internal val Purple80 = Color(0xFFD0BCFF)
internal val PurpleGrey80 = Color(0xFFCCC2DC)
internal val Pink80 = Color(0xFFEFB8C8)

internal val Purple40 = Color(0xFF6650a4)
internal val PurpleGrey40 = Color(0xFF625b71)
internal val Pink40 = Color(0xFF7D5260)

internal val ContainerBackground = Color(0xFFF6FAFE)
internal val ContainerTextColor = Color(0xFF181C20)
internal val DefaultTextColor = Color.Black
internal val DividerColor = Color(0xFFC1C7CE)
